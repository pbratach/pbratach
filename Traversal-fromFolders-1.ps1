...
function Restart-Archive
{
  ...
  Start-Sleep 3
  #$Sql.CommandText = "UPDATE archive SET enabled=1 WHERE id=$id"
  #$res = $Sql.ExecuteNonQuery()

  $sql = "UPDATE archive SET enabled=1 WHERE id=$id"
  $res = "$sql" | & "$dbProgram" --user=$user --password=$pass -b -s --database=$database --ssl
...
# Remove the comment on the line below to remove any existing log files
